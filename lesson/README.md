# Per-clip demo briefs

These are the `DEMO_BRIEF.md` files from `original_prompt_2.md`, extracted as
standalone files for easy reference while recording.

Files are numbered in clip-chronological order (which differs from prompt 2's
internal branch numbering). The companion branch name is shown next to each.

| # | Clip(s) | File | Branch in prompt 2 |
|---|---------|------|--------------------|
| 01 | M1 C2 / C3 / C4 | [01-m1-bootstrap-planning.md](01-m1-bootstrap-planning.md) | `demo/m1-bootstrap-planning` |
| 02 | M2 C1 | [02-m2-c1-class-component-audit.md](02-m2-c1-class-component-audit.md) | `demo/m2-c1-class-component-audit` |
| 03 | M2 C2 | [03-m2-c2-class-to-functional.md](03-m2-c2-class-to-functional.md) | `demo/m2-c2-class-to-functional` |
| 04 | M2 C3 | [04-m2-c3-architectural-refactoring.md](04-m2-c3-architectural-refactoring.md) | `demo/m2-c3-architectural-refactoring` |
| 05 | M2 C4 | [05-m2-c4-performance.md](05-m2-c4-performance.md) | `demo/m2-c4-performance` |
| 06 | M2 C5 | [06-m2-c5-modernize-dependencies.md](06-m2-c5-modernize-dependencies.md) | `demo/m2-c5-modernize-dependencies` |
| 07 | M3 C1 | [07-m3-c1-dead-endpoints.md](07-m3-c1-dead-endpoints.md) | `demo/m3-c1-dead-endpoints` |
| 08 | M3 C3 | [08-m3-c3-cross-cutting-plan.md](08-m3-c3-cross-cutting-plan.md) | continues `demo/m3-c1-dead-endpoints` |
| 09 | M3 C4 | [09-m3-c4-design-consistency.md](09-m3-c4-design-consistency.md) | `demo/m3-c4-design-consistency` |
| 10 | M3 C5 | [10-m3-c5-example-driven-migration.md](10-m3-c5-example-driven-migration.md) | `demo/m3-c5-example-driven-migration` |
| 11 | M4 C1 | [11-m4-c1-regression-risks.md](11-m4-c1-regression-risks.md) | `demo/m4-c1-regression-risks` |
| 12 | M4 C2 | [12-m4-c2-git-discipline.md](12-m4-c2-git-discipline.md) | `demo/m4-c2-git-discipline` |
| 13 | M4 C3 | [13-m4-c3-cicd-quality-gates.md](13-m4-c3-cicd-quality-gates.md) | `demo/m4-c3-cicd-quality-gates` |
| 14 | M5 C3 | [14-m5-c3-sustaining.md](14-m5-c3-sustaining.md) | `demo/m5-c3-sustaining` |

## Note on improvements

These briefs are extracted **verbatim** from prompt 2 as it stands today. The
review plan at `~/.claude/plans/prompt-1-twinkly-quill.md` lists 9 pedagogical
improvements (F1–F9) to apply to each brief. None are applied yet.
